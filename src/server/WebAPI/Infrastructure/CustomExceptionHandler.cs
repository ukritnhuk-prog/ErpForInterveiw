    ...
    ILogger<CustomExceptionHandler> logger) : IExceptionHandler
{
        ...
        CancellationToken cancellationToken)
    {
        var (status, message) = exception switch
        {
            ...
            DbUpdateConcurrencyException =>
                (StatusCodes.Status409Conflict,
                    "This record changed or was deleted. Refresh and try again."),
            DbUpdateException { InnerException: SqlException { Number: 547 } } =>
                (StatusCodes.Status400BadRequest,
                    "The change conflicts with a department relationship or a data constraint. " +
        ...
        if (status == 500)
        {
            ...
            logger.LogError(
                "Unhandled {ExceptionType}. Trace: {TraceId}",
        ...
        await context.Response.WriteAsJsonAsync(
            Response<object>.Failure(message),
            cancellationToken);
